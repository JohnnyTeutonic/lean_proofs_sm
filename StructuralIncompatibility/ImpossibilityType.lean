/-
  ...
  **See also**: `ObstructionToSymmetry.lean` — The P functor that maps
  obstructions to forced positive structures (symmetries). Each mechanism
  type forces a specific symmetry type:
    - diagonal → Z₂ (discrete)
    - resource → SO(3) (continuous)  
    - structural → Sₙ (permutation)
    - parametric → Gauge (∞-dimensional)
  
  Composing obstructions yields product symmetries (richer structure).
