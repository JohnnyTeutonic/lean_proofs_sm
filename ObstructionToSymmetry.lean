/-
  ...
  This file implements OTS1/OTS2 upgrades from RIGOR_UPGRADE_PLAN.md:
  
  **OTS1**: This file is a DIDACTIC BRIDGE, not authoritative definitions.
           The authoritative P functor is in `InverseNoetherV2.lean`.
           Types here are simplified versions for pedagogical clarity.
  
  **OTS2**: `composition_law_structural` theorem added showing how
           obstruction composition relates to symmetry join.
...
/-! ## OTS2: Composition Law as Structural Theorem -/

/-- **OTS2**: The composition law is a STRUCTURAL theorem.
    
    When obstructions compose:
    - Mechanisms join via dominance ordering
    - Witnesses form products
    - Gauge dominates everything
    
    This is NOT a definition but a THEOREM about the structure. -/
theorem composition_law_gauge_dominates (o₁ o₂ : Obstruction) :
    o₁.mechanism = .parametric → P (o₁.compose o₂).mechanism = .gauge := by
  intro h
  simp only [Obstruction.compose, P, mechToQuotient, quotientToSym, h]
...
/-- **OTS2**: Witness product theorem -/
theorem witness_product_theorem (o₁ o₂ : Obstruction) :
    (P_obj (o₁.compose o₂)).carrier = (o₁.witness × o₂.witness) := rfl

/-- **OTS1**: Bridge to InverseNoetherV2 terminology.
    
    This file's `Obstruction` corresponds to `InverseNoetherV2.NegObj`.
    This file's `SymObj` corresponds to `InverseNoetherV2.PosObj`.
    This file's `P` corresponds to `InverseNoetherV2.P_obj.stype ∘ quotient`. -/
def authoritative_reference : String :=
  "For authoritative P functor, see InverseNoetherV2.P_obj"
