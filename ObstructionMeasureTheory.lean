...
-/
structure ObservableSpace (Ω : Type _) where
...
def is_observable {Ω : Type _} (space : ObservableSpace Ω) (x : Ω) : Prop :=
  @MeasurableSet Ω space.observable {x}

/-- A point is in the obstruction spectrum if it's definable but not observable -/
def in_obstruction_spectrum {Ω : Type _} (space : ObservableSpace Ω) (x : Ω) : Prop :=
...
theorem binary_quotient {Ω : Type _} (space : ObservableSpace Ω) (x : Ω) 
